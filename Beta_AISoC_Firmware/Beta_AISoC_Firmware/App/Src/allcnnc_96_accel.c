...
#define ALLCNNC96_SIZE_OPT __attribute__((noinline, optimize("Os")))

#define ALLCNNC96_LAYER_COUNT               5U
#define ALLCNNC96_DMAC_WRITE_WEIGHT         1U
#define ALLCNNC96_DMAC_READ_WEIGHT          2U
#define ALLCNNC96_SUBMIT_WAIT_LIMIT         10000000U
#define ALLCNNC96_IRQ_WAIT_LIMIT            100000000U
#define ALLCNNC96_HRAM_DRAIN_WAIT_LIMIT     50000000U
#define ALLCNNC96_HRAM_DRAIN_STABLE_READS   128U
#define ALLCNNC96_VIDEO_WARMUP_MS           100U
#define ALLCNNC96_FINAL_ROW_BYTES           12U
#define ALLCNNC96_PARAM_BYTES_RESERVED      316460U
#define ALLCNNC96_ACTIVATION_BYTES_RESERVED 184752U
#define ALLCNNC96_LOWER_WRAP_MS             ((uint32_t)(0x100000000ULL / CYCLES_PER_MS))
#define ALLCNNC96_LOWER_WRAP_REM            ((uint32_t)(0x100000000ULL % CYCLES_PER_MS))
...
} AllCNNC96_RunContext_t;

static const AllCNNC96_Layer_t s_allcnnc96_layers[ALLCNNC96_LAYER_COUNT] = {
    { "Conv1", 96U,   3U,  32U, 3U, 2U, 1U, 3U, 1U, 8U,      0U,      0U,    864U,  27648U,  -26, 0, 21353530, 31U, -128, -128, 127, false },
    { "Conv2", 48U,  32U,  64U, 3U, 2U, 1U, 4U, 1U, 8U,  27648U,    992U,  19424U, 101376U, -128, 0,  9342586, 31U, -128, -128, 127, false },
    { "Conv3", 24U,  64U, 128U, 3U, 2U, 1U, 4U, 1U, 8U, 101376U,  19680U,  93408U, 138240U, -128, 0,  3857325, 31U, -128, -128, 127, false },
    { "Conv4", 12U, 128U, 192U, 3U, 1U, 1U, 4U, 1U, 8U, 138240U,  93920U, 315104U, 156672U, -128, 0,  1564635, 31U, -128, -128, 127, false },
    { "Conv5", 12U, 192U,   3U, 1U, 1U, 0U, 4U, 1U, 3U, 156672U, 315872U, 316448U, 184320U, -128, 0,   582206, 31U,   21, -128, 127, true },
};

static volatile bool s_allcnnc96_irq_seen;
static uint8_t s_allcnnc96_gap_row[ALLCNNC96_FINAL_ROW_BYTES];
static volatile int32_t s_allcnnc96_gap_sink;
static const char *const s_allcnnc96_labels[] = { "paper", "rock", "scissors" };

static ALLCNNC96_SIZE_OPT void print_u32_dec(uint32_t value)
{
    char text[10];
    uint32_t pos = 0U;
...
static ALLCNNC96_SIZE_OPT void print_i32_dec(int32_t value)
{
    if (value < 0) {
...
static ALLCNNC96_SIZE_OPT tick_t ticks_elapsed(tick_t start, tick_t end)
{
    tick_t diff;
...
static ALLCNNC96_SIZE_OPT tick_t ticks_add(tick_t a, tick_t b)
{
    tick_t sum;
...
static ALLCNNC96_SIZE_OPT uint32_t ticks_to_ms(tick_t ticks)
{
    uint32_t ms = ticks.upper * ALLCNNC96_LOWER_WRAP_MS + ticks.lower / CYCLES_PER_MS;
    uint32_t rem = ticks.upper * ALLCNNC96_LOWER_WRAP_REM + ticks.lower % CYCLES_PER_MS;

    ms += rem / CYCLES_PER_MS;
    return ms;
}

static ALLCNNC96_SIZE_OPT uint8_t tick_cycles_to_digits(tick_t ticks,
                                                         uint8_t *digits,
                                                         uint8_t max_digits)
{
    uint8_t count = 0U;
...
static ALLCNNC96_SIZE_OPT void print_tick_cycles(tick_t ticks)
{
    uint8_t digits[20];
...
static ALLCNNC96_SIZE_OPT void print_tick_metric(tick_t ticks)
{
...
static ALLCNNC96_SIZE_OPT uint32_t ticks_to_u32_saturated(tick_t ticks)
{
    return (ticks.upper != 0U) ? 0xFFFFFFFFU : ticks.lower;
}

static ALLCNNC96_SIZE_OPT void print_cycles_per_unit(tick_t ticks, uint32_t units)
{
    uint32_t cycles;
...
static ALLCNNC96_SIZE_OPT uint32_t layer_ofwidth(const AllCNNC96_Layer_t *layer)
{
...
static ALLCNNC96_SIZE_OPT uint32_t layer_ofmap_bytes(const AllCNNC96_Layer_t *layer)
{
...
static ALLCNNC96_SIZE_OPT uint32_t layer_macs(const AllCNNC96_Layer_t *layer)
{
...
static ALLCNNC96_SIZE_OPT uint32_t total_conv_macs(void)
{
    uint32_t total = 0U;

    for (uint32_t i = 0U; i < ALLCNNC96_LAYER_COUNT; i++) {
        total += layer_macs(&s_allcnnc96_layers[i]);
    }

    return total;
}

static ALLCNNC96_SIZE_OPT void print_cnn_status(const char *prefix, uint32_t status)
{
    Uart_print(prefix);
...
static ALLCNNC96_SIZE_OPT void print_hyperram_status(const char *name,
...
static ALLCNNC96_SIZE_OPT bool wait_hyperram_master_idle(const char *name,
                                                          HyperRAM_Driver_t *drv)
{
    uint32_t wait = ALLCNNC96_HRAM_DRAIN_WAIT_LIMIT;
    uint32_t stable = 0U;

    while (wait > 0U) {
        if (HyperRAM_is_start_ready(drv)) {
            stable++;
            if (stable >= ALLCNNC96_HRAM_DRAIN_STABLE_READS) {
                return true;
            }
        } else {
            stable = 0U;
...
static void allcnnc96_irq_handler(uint32_t irq_bit, void *context)
{
    (void)irq_bit;
    (void)context;
    s_allcnnc96_irq_seen = true;
}

static ALLCNNC96_SIZE_OPT void fill_layer_config(const AllCNNC96_Layer_t *layer,
...
static ALLCNNC96_SIZE_OPT void print_layer_config(const AllCNNC96_Layer_t *layer)
{
...
static ALLCNNC96_SIZE_OPT bool run_layer(uint8_t idx,
                                          const AllCNNC96_Layer_t *layer,
                                          bool verbose,
                                          tick_t *elapsed)
{
    CNN_Accel_LayerConfig_t config;
    tick_t start_tick;
    tick_t end_tick;
    uint32_t wait = ALLCNNC96_IRQ_WAIT_LIMIT;
    uint32_t status;
    bool done_seen = false;

    fill_layer_config(layer, &config);
    s_allcnnc96_irq_seen = false;
    ...
    if (!CNN_Accel_submit_layer_config(&cnn_accel, &config, ALLCNNC96_SUBMIT_WAIT_LIMIT)) {
    ...
    while (wait > 0U) {
        status = CNN_Accel_get_status(&cnn_accel);
        if (s_allcnnc96_irq_seen || ((status & CNN_ACCEL_STATUS_DONE_Msk) != 0U)) {
            done_seen = true;
            break;
        }
        wait--;
    ...
    if (!CNN_Accel_wait_idle(&cnn_accel, ALLCNNC96_SUBMIT_WAIT_LIMIT)) {
...
static ALLCNNC96_SIZE_OPT void print_layer_profile(const AllCNNC96_Layer_t *layer,
                                                    tick_t ticks)
{
...
static ALLCNNC96_SIZE_OPT bool run_global_avgpool(W95_HandleTypeDef *w95_read,
                                                   const AllCNNC96_Layer_t *layer,
                                                   bool verbose,
                                                   int32_t *logits_out,
                                                   tick_t *elapsed)
{
    ...
    if (h > ALLCNNC96_FINAL_ROW_BYTES) {
        return false;
    }
    ...
    for (uint32_t c = 0U; c < layer->ofchannel; c++) {
        ...
        for (uint32_t y = 0U; y < h; y++) {
            W95_MemoryRead(w95_read, ch_base + y * h, s_allcnnc96_gap_row, h, true);
            for (uint32_t x = 0U; x < h; x++) {
                sum += (int32_t)(int8_t)s_allcnnc96_gap_row[x] - (int32_t)layer->zpy;
            }
        }
...
static ALLCNNC96_SIZE_OPT bool begin_run_context(AllCNNC96_RunContext_t *context,
                                                     bool use_camera_ifmap)
{
    ...
    if (use_camera_ifmap) {
        delay(ALLCNNC96_VIDEO_WARMUP_MS);
    }

    if (!CNN_Accel_attach_irq(allcnnc96_irq_handler, 0, IRQ_PRIORITY_DEFAULT)) {
        Uart_println("  CNN IRQ attach failed.");
        return false;
    }

    CNN_Accel_enable_irq();
    HyperRAM_set_dmac_weights(&hyperram0, ALLCNNC96_DMAC_WRITE_WEIGHT, ALLCNNC96_DMAC_READ_WEIGHT);
    HyperRAM_set_dmac_weights(&hyperram1, ALLCNNC96_DMAC_WRITE_WEIGHT, ALLCNNC96_DMAC_READ_WEIGHT);
    return true;
}

static ALLCNNC96_SIZE_OPT void end_run_context(const AllCNNC96_RunContext_t *context)
{
...
static ALLCNNC96_SIZE_OPT bool run_model_once(AllCNNC96_RunContext_t *context,
                                               bool verbose,
                                               bool use_camera_ifmap,
                                               int32_t *logits_out,
                                               tick_t *conv_ticks,
                                               ...
                                               tick_t *wall_ticks)
{
    ...
    for (uint8_t i = 0U; ok && (i < ALLCNNC96_LAYER_COUNT); i++) {
        tick_t elapsed;
        ...
        ok = run_layer(i, &s_allcnnc96_layers[i], verbose, &elapsed);
        if (use_camera_ifmap && (i == 0U)) {
            VideoStreaming_set_grant_request(&video_streaming, false);
        }
        HyperRAM_set_accel_mode(&hyperram0, false);
        ...
        if (ok) {
            *conv_ticks = ticks_add(*conv_ticks, elapsed);
            if (verbose) {
                print_layer_profile(&s_allcnnc96_layers[i], elapsed);
            }
        }
    }
    ...
    if (ok) {
        ...
        ok = run_global_avgpool(&context->w95_h1_read,
                                &s_allcnnc96_layers[ALLCNNC96_LAYER_COUNT - 1U],
                                verbose,
                                logits_out,
                                &elapsed);
        if (ok) {
...
bool AllCNNC96_Accel_RunTimingOnly(void)
{
    bool ok;
    AllCNNC96_RunContext_t context;
    ...
    Uart_println("");
    Uart_println("=== Run ALL-CNN-C-96 accelerator timing ===");
    ...
    print_u32_dec(ALLCNNC96_LAYER_COUNT);
    Uart_println("");
    Uart_print("  Conv head classes        : ");
    print_u32_dec(s_allcnnc96_layers[ALLCNNC96_LAYER_COUNT - 1U].ofchannel);
    Uart_println("");
    Uart_print("  HR0 filter/bias reserved : ");
    print_u32_dec(ALLCNNC96_PARAM_BYTES_RESERVED);
    Uart_println(" bytes");
    Uart_print("  HR1 activation reserved  : ");
    print_u32_dec(ALLCNNC96_ACTIVATION_BYTES_RESERVED);
    Uart_println(" bytes");
    ...
    if (ok) {
        ...
        Uart_println("");
        Uart_println("ALL-CNN-C-96 timing summary:");
    ...
    Uart_println(ok ? "ALL-CNN-C-96 timing -> DONE" : "ALL-CNN-C-96 timing -> FAIL");
    return ok;
}

static ALLCNNC96_SIZE_OPT uint8_t argmax_logits(const int32_t *logits)
{
    uint8_t best = 0U;
...
static ALLCNNC96_SIZE_OPT void print_result_line(const int32_t *logits,
                                                  tick_t conv_ticks,
                                                  tick_t cpu_ticks,
                                                  tick_t wall_ticks)
{
    ...
    Uart_print(s_allcnnc96_labels[label]);
    Uart_print(" logits=");
...
bool AllCNNC96_Accel_RunCameraResultLoop(void)
{
    AllCNNC96_RunContext_t context;
    bool ok;
    ...
    Uart_println("");
    end_run_context(&context);
    Uart_println(ok ? "ALL-CNN-C-96 camera loop -> DONE" : "ALL-CNN-C-96 camera loop -> FAIL");
    return ok;
}
