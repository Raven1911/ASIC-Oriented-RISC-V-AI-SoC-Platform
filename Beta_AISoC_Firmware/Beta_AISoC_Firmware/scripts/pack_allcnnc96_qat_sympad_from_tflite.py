...
TOTAL_SIZE = 316460
MODEL_FILE = "rps_cnn_96_s2_qat_sympad_int8_per_layer.tflite"
PARAMS_FILE = "rps_cnn_96_s2_qat_sympad_fixed_params_int_only.json"
ACCEL_M = 2
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Pack RPS ALL-CNN-C-96 QAT SymPad INT8 TFLite weights/biases for accelerator HyperRAM0."
    )
    parser.add_argument("--model-dir", type=Path, required=True)
    parser.add_argument("--tflite", type=Path)
    parser.add_argument("--params", type=Path)
    parser.add_argument("--bin-out", type=Path, default=Path("Build/allcnnc_96_qat_sympad_packed_weights.bin"))
    parser.add_argument("--hex-out", type=Path, default=Path("Build/allcnnc_96_qat_sympad_packed_weights.hex"))
